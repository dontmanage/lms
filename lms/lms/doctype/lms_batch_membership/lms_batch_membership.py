...
class LMSBatchMembership(Document):
	...
	def validate_membership_in_same_batch(self):
		...
		previous_membership = dontmanage.db.get_value(
			"LMS Batch Membership", filters, fieldname=["member_type", "member"], as_dict=1
		)

		if previous_membership:
			member_name = dontmanage.db.get_value("User", self.member, "full_name")
	...
	def validate_membership_in_different_batch_same_course(self):
		...
		course = dontmanage.db.get_value("LMS Batch", self.batch, "course")
...
def update_current_membership(batch, course, member):
	...
	for membership in all_memberships:
		dontmanage.db.set_value("LMS Batch Membership", membership.name, "is_current", 0)

	current_membership = dontmanage.get_all(
