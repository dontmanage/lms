...
class Cohort(Document):
	def get_url(self):
		return f"{dontmanage.utils.get_url()}/courses/{self.course}/cohorts/{self.slug}"

	def get_subgroups(self, include_counts=False, sort_by=None):
		names = dontmanage.get_all("Cohort Subgroup", filters={"cohort": self.name}, pluck="name")
		subgroups = [dontmanage.get_cached_doc("Cohort Subgroup", name) for name in names]
	...
	def _get_count(self, doctype, **kw):
		filters = {"cohort": self.name, **kw}
		return dontmanage.db.count(doctype, filters=filters)
	...
	def get_subgroup(self, slug):
		...
		name = dontmanage.db.get_value("Cohort Subgroup", q, "name")
		return name and dontmanage.get_doc("Cohort Subgroup", name)

	def get_mentor(self, email):
		q = dict(cohort=self.name, email=email)
		name = dontmanage.db.get_value("Cohort Mentor", q, "name")
		return name and dontmanage.get_doc("Cohort Mentor", name)
