...
class TestInviteRequest(unittest.TestCase):
	...
	def test_create_invite_request(self):
		if dontmanage.db.exists("Invite Request", {"invite_email": "test_invite@example.com"}):
			invite = dontmanage.db.get_value(
				"Invite Request",
	...
	def test_create_invite_request_update(self):
		if dontmanage.db.exists("Invite Request", {"invite_email": "test_invite@example.com"}):
			...
			data = {
				...
				"invite_code": dontmanage.db.get_value(
	...
	def tearDownClass(self):
		if dontmanage.db.exists("User", "test_invite@example.com"):
			dontmanage.delete_doc("User", "test_invite@example.com")

		invite_request = dontmanage.db.exists(
			"Invite Request", {"invite_email": "test_invite@example.com"}
		)
		if invite_request:
			dontmanage.delete_doc("Invite Request", invite_request)
