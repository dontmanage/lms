...
class LMSBatch(Document):
	...
	def validate_if_mentor(self):
		if not is_mentor(self.course, dontmanage.session.user):
			course_title = dontmanage.db.get_value("LMS Course", self.course, "title")
			dontmanage.throw(_("You are not a mentor of the course {0}").format(course_title))
	...
	def is_member(self, email, member_type=None):
		...
		return dontmanage.db.exists("LMS Batch Membership", filters)
	...
	def get_membership(self, email):
		...
		return dontmanage.get_doc("LMS Batch Membership", name)
	...
	doc = dontmanage.get_doc(
		{
			"doctype": "LMS Message",
			"batch": batch,
			"author": dontmanage.session.user,
			"message": message,
		}
	)
...
def switch_batch(course_name, email, batch_name):
	...
	membership = dontmanage.get_last_doc(
		"LMS Batch Membership", filters={"course": course_name, "member": email}
	)

	batch = dontmanage.get_doc("LMS Batch", batch_name)
	if not batch:
	...
	old_batch = dontmanage.get_doc("LMS Batch", membership.batch)
	...
	for name in dontmanage.db.get_all("Exercise Submission", filters=filters, pluck="name"):
		doc = dontmanage.get_doc("Exercise Submission", name)
