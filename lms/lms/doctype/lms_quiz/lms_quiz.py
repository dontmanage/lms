		...
		if not len(correct_options):
			dontmanage.throw(
				_("At least one option must be correct for this question: {0}").format(
					dontmanage.bold(question.question)
				)
			)
...
def quiz_summary(quiz, results):
	...
	for result in results:
		...
		result["question"] = dontmanage.db.get_value(
...
def save_quiz(quiz_title, questions, quiz):
	if quiz:
		doc = dontmanage.get_doc("LMS Quiz", quiz)
	else:
	...
	for index, row in enumerate(json.loads(questions)):
		if row["question_name"]:
			question_doc = dontmanage.get_doc("LMS Quiz Question", row["question_name"])
		else:
			question_doc = dontmanage.get_doc(
				{
...
def check_choice_answers(question, answer):
	...
	question_details = dontmanage.db.get_value(
