...
def autosave_section(section, code):
	...
	doc = dontmanage.get_doc(
		doctype="Code Revision", section=section, code=code, author=dontmanage.session.user
	)
	doc.insert()
...
def submit_solution(exercise, code):
	...
	"""
	ex = dontmanage.get_doc("LMS Exercise", exercise)
	if not ex:
		return
	doc = ex.submit(code)
...
def save_current_lesson(course_name, lesson_name):
	...
	name = dontmanage.get_value(
		doctype="LMS Batch Membership",
		filters={"course": course_name, "member": dontmanage.session.user},
		fieldname="name",
	)
	if not name:
		return
	doc = dontmanage.get_doc("LMS Batch Membership", name)
...
def join_cohort(course, cohort, subgroup, invite_code):
	...
	course_doc = dontmanage.get_doc("LMS Course", course)
...
def approve_cohort_join_request(join_request):
	r = dontmanage.get_doc("Cohort Join Request", join_request)
	sg = r and dontmanage.get_doc("Cohort Subgroup", r.subgroup)
	if not sg or r.status not in ["Pending", "Accepted"]:
		return {"ok": False, "error": "Invalid Join Request"}
	if (
		not sg.is_manager(dontmanage.session.user) and "System Manager" not in dontmanage.get_roles()
	):
...
def add_mentor_to_subgroup(subgroup, email):
	try:
		sg = dontmanage.get_doc("Cohort Subgroup", subgroup)
	except dontmanage.DoesNotExistError:
		return {"ok": False, "error": f"Invalid subgroup: {subgroup}"}

	if (
		not sg.get_cohort().is_admin(dontmanage.session.user)
