...
def generate_slug(title, doctype):
	result = dontmanage.get_all(doctype, fields=["name"])
...
def get_membership(course, member, batch=None):
	...
	membership = dontmanage.db.get_value(
	...
	if membership and membership.batch:
		membership.batch_title = dontmanage.db.get_value("LMS Batch", membership.batch, "title")
	return membership
...
def get_chapters(course):
	...
	if not course:
		return []
	chapters = dontmanage.get_all(
		"Chapter Reference", {"parent": course}, ["idx", "chapter"], order_by="idx"
	)
	for chapter in chapters:
		chapter_details = dontmanage.db.get_value(
			"Course Chapter",
...
def get_lesson_details(chapter):
	lessons = []
	lesson_list = dontmanage.get_all(
		"Lesson Reference", {"parent": chapter.name}, ["lesson", "idx"], order_by="idx"
	)

	for row in lesson_list:
		lesson_details = dontmanage.db.get_value(
			"Course Lesson",
			row.lesson,
			[
				"name",
...
def get_tags(course):
	tags = dontmanage.db.get_value("LMS Course", course, "tags")
	return tags.split(",") if tags else []


def get_instructors(course):
	instructor_details = []
	instructors = dontmanage.get_all(
		"Course Instructor", {"parent": course}, ["instructor"], order_by="idx"
	)
	if not instructors:
		instructors = dontmanage.db.get_value("LMS Course", course, "owner").split(" ")
...
def get_students(course, batch=None):
	...
	return dontmanage.get_all("LMS Batch Membership", filters, ["member"])
...
def get_reviews(course):
	...
	for review in reviews:
		...
		review.owner_details = dontmanage.db.get_value(
...
def get_sorted_reviews(course):
	rating_count = rating_percent = dontmanage._dict()
...
def is_certified(course):
	certificate = dontmanage.get_all(
		"LMS Certificate", {"member": dontmanage.session.user, "course": course}
	)
	if len(certificate):
...
def get_lesson_index(lesson_name):
	...
	lesson = dontmanage.db.get_value(
		"Lesson Reference", {"lesson": lesson_name}, ["idx", "parent"], as_dict=True
	)
	if not lesson:
		return "1.1"

	chapter = dontmanage.db.get_value(
...
def get_batch(course, batch_name):
	return dontmanage.get_all("LMS Batch", {"name": batch_name, "course": course})
...
def get_progress(course, lesson):
	return dontmanage.db.get_value(
		"LMS Course Progress",
		{"course": course, "owner": dontmanage.session.user, "lesson": lesson},
		["status"],
	)
...
def is_cohort_staff(course, user_email):
	...
	return dontmanage.db.exists(staff) or dontmanage.db.exists(mentor)


def get_mentors(course):
	"""Returns the list of all mentors for this course."""
	course_mentors = []
	mentors = dontmanage.get_all("LMS Course Mentor Mapping", {"course": course}, ["mentor"])
	for mentor in mentors:
		member = dontmanage.db.get_value(
			"User", mentor.mentor, ["name", "username", "full_name", "user_image"]
		)
		member.batch_count = dontmanage.db.count(
...
def is_eligible_to_review(course, membership):
	...
	if dontmanage.db.count(
		"LMS Course Review", {"course": course, "owner": dontmanage.session.user}
	):
		return False
	return True
...
def get_course_progress(course, member=None):
	...
	if not lesson_count:
		return 0
	completed_lessons = dontmanage.db.count(
		"LMS Course Progress",
		{"course": course, "owner": member or dontmanage.session.user, "status": "Complete"},
	)
...
def is_instructor(course):
	return (
		len(list(filter(lambda x: x.name == dontmanage.session.user, get_instructors(course))))
		> 0
	)
...
def get_signup_optin_checks():
	...
	for check in checks:
		if dontmanage.db.get_single_value("LMS Settings", check):
			page = dontmanage.db.get_single_value("LMS Settings", mapper[check].get("page_name"))
			route = dontmanage.db.get_value("Web Page", page, "route")
...
def get_popular_courses():
	courses = dontmanage.get_all("LMS Course", {"published": 1, "upcoming": 0})
	course_membership = []
	...
	for course in courses:
		course_membership.append(
			{
				"course": course.name,
				"members": cint(dontmanage.db.count("LMS Batch Membership", {"course": course.name})),
			}
		)
...
def get_evaluation_details(course, member=None):
	info = dontmanage.db.get_value(
		"LMS Course",
		course,
		["grant_certificate_after", "max_attempts", "duration"],
		as_dict=True,
	)
	request = dontmanage.db.get_value(
		"LMS Certificate Request",
		{
			"course": course,
			"member": member or dontmanage.session.user,
			"date": [">=", getdate()],
		},
...
def format_number(number):
	...
	if number_reduced < 1:
		return number
	return f"{dontmanage.utils.flt(number_reduced, 1)}k"


def first_lesson_exists(course):
	first_chapter = dontmanage.db.get_value(
		"Chapter Reference", {"parent": course, "idx": 1}, "name"
	)
	if not first_chapter:
		return False

	first_lesson = dontmanage.db.get_value(
...
def has_course_instructor_role(member=None):
	return dontmanage.db.get_value(
		"Has Role",
		{"parent": member or dontmanage.session.user, "role": "Course Creator"},
		"name",
	)
...
def can_create_courses(member=None):
	...
	portal_course_creation = dontmanage.db.get_single_value(
		"LMS Settings", "portal_course_creation"
	)
	return dontmanage.session.user != "Guest" and (
...
def has_course_moderator_role(member=None):
	return dontmanage.db.get_value(
		"Has Role",
		{"parent": member or dontmanage.session.user, "role": "Moderator"},
		"name",
	)
...
def validate_image(path):
	if path and "/private" in path:
		file = dontmanage.get_doc("File", {"file_url": path})
		file.is_private = 0
...
def create_notification_log(doc, method):
	...
	course = dontmanage.db.get_value("Course Lesson", topic.reference_docname, "course")
	instructors = dontmanage.db.get_all(
		"Course Instructor", {"parent": course}, pluck="instructor"
	)

	notification = dontmanage._dict(
		{
...
def get_lesson_count(course):
	lesson_count = 0
	chapters = dontmanage.get_all("Chapter Reference", {"parent": course}, ["chapter"])
	for chapter in chapters:
		lesson_count += dontmanage.db.count("Lesson Reference", {"parent": chapter.chapter})

	return lesson_count


def check_profile_restriction():
	return dontmanage.db.get_single_value("LMS Settings", "force_profile_completion")


def get_restriction_details():
	user = dontmanage.db.get_value(
		"User", dontmanage.session.user, ["profile_complete", "username"], as_dict=True
	)
	return {
		"restrict": not user.profile_complete,
		"username": user.username,
		"prefix": dontmanage.get_hooks("profile_url_prefix")[0] or "/users/",
	}
...
def has_lessons(course):
	lesson_exists = False
	chapter_exists = dontmanage.db.get_value(
		"Chapter Reference", {"parent": course.name}, ["name", "chapter"], as_dict=True
	)

	if chapter_exists:
		lesson_exists = dontmanage.db.exists(
...
def get_course_completion_data():
	all_membership = dontmanage.db.count("LMS Batch Membership")
	completed = dontmanage.db.count("LMS Batch Membership", {"progress": ["like", "%100%"]})
