...
from dontmanage.utils import cint, escape_html, random_string
...
class CustomUser(User):
	...
	def validate_username_characters(self):
		...
		if self.is_new():
			...
			if not self.username:
				dontmanage.throw(_("Username already exists."))

			if regex.search(self.username):
				dontmanage.throw(_("Username can only contain alphabets, numbers and underscore."))

			if other_conditions:
				if "-" in self.username:
					dontmanage.throw(_("Username cannot contain a Hyphen(-)"))
				else:
					dontmanage.throw(_("First and Last character of username cannot be Underscore(_)."))

			if len(self.username) < 4:
				dontmanage.throw(_("Username cannot be less than 4 characters"))

	def get_username_from_first_name(self):
		return dontmanage.scrub(self.first_name) + str(random.randint(0, 99))
	...
	def validate_skills(self):
		...
		for skill in self.skill:
			...
			if not skill.skill_name in unique_skills:
				...
				dontmanage.throw(_("Skills must be unique"))

	def validate_completion(self):
		if dontmanage.db.get_single_value("LMS Settings", "force_profile_completion"):
			all_fields_have_value = True
			profile_mandatory_fields = dontmanage.get_hooks("profile_mandatory_fields")
			docfields = dontmanage.get_meta(self.doctype).fields
	...
	def get_user_reviews(self):
		...
		return dontmanage.get_all("LMS Course Review", {"owner": self.name})
	...
	def get_mentored_courses(self):
		...
		for map in mapping:
			if dontmanage.db.get_value("LMS Course", map.course, "published"):
...
def get_course_membership(member=None, member_type=None):
	...
	filters = {"member": member or dontmanage.session.user}
	if member_type:
		filters["member_type"] = member_type

	return dontmanage.get_all("LMS Batch Membership", filters, ["name", "course", "progress"])
...
def get_authored_courses(member=None, only_published=True):
	...
	courses = dontmanage.get_all(
		"Course Instructor", {"instructor": member or dontmanage.session.user}, ["parent"]
	)

	for course in courses:
		detail = dontmanage.db.get_value(
			"LMS Course",
			course.parent,
			[
				"name",
...
def sign_up(email, full_name, verify_terms, user_category):
	if is_signup_disabled():
		dontmanage.throw(_("Sign Up is disabled"), _("Not Allowed"))
	...
	default_role = dontmanage.db.get_value("Portal Settings", None, "default_role")
	if default_role:
...
def set_country_from_ip(login_manager=None, user=None):
	...
	user_country = dontmanage.db.get_value("User", user, "country")
	# if user_country:
	#    return
	dontmanage.db.set_value("User", user, "country", get_country_code())
	return
...
def get_country_code():
	...
	try:
		...
		if data.get("status") != "fail":
			return dontmanage.db.get_value("Country", {"code": data.get("countryCode")}, "name")
	except Exception:
		pass
	return
...
def on_session_creation(login_manager):
	...
	) and dontmanage.db.get_single_value("LMS Settings", "default_home"):
		dontmanage.local.response["home_page"] = "/courses"
...
def save_role(user, role, value):
	if cint(value):
		...
		dontmanage.db.delete("Has Role", {"parent": user, "role": role})
	return True
