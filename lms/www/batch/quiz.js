dontmanage.ready(() => {
	...
	$(".copy-quiz-id").click((e) => {
		dontmanage.utils.copy_to_clipboard($(e.currentTarget).data("name"));
	});
...
const save_question = (e) => {
	if (!$("#quiz-title").text()) {
		dontmanage.throw(__("Quiz Title is mandatory."));
	}
...
const validate_mandatory = (details, correct_options, possibilities) => {
	if (details["type"] == "Choices") {
		if (!details["option_1"] || !details["option_2"]) {
			...
			dontmanage.throw(__("Each question must have at least two options."));
		}
