dontmanage.ready(() => {
	...
	$(document).on("click", ".copy-link", (e) => {
		dontmanage.utils.copy_to_clipboard($(e.currentTarget).data("link"));
...
const save_current_lesson = () => {
	if ($(".title").hasClass("is-member")) {
		dontmanage.call("lms.lms.api.save_current_lesson", {
...
const make_editor = () => {
	this.code_field_group = new dontmanage.ui.FieldGroup({
		fields: [
			{
				fieldname: "code_md",
				fieldtype: "Code",
	...
	$("#body .dontmanage-control").removeClass("hide-control");
