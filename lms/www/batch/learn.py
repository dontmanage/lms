...
def get_context(context):
	...
	chapter_index = dontmanage.form_dict.get("chapter")
	lesson_index = dontmanage.form_dict.get("lesson")
	lesson_number = f"{chapter_index}.{lesson_index}"
	context.lesson_index = lesson_index
	context.chapter = dontmanage.db.get_value(
	...
	if not context.lesson:
		context.lesson = dontmanage._dict()

	if dontmanage.form_dict.get("edit"):
		if not instructor and not has_course_moderator_role():
			raise dontmanage.PermissionError(_("You do not have permission to access this page."))
...
def get_page_extensions(context):
	...
	classnames = dontmanage.get_hooks("lms_lesson_page_extensions") or default_value
	extensions = [dontmanage.get_attr(name)() for name in classnames]
	for e in extensions:
