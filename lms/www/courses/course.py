...
def get_context(context):
	...
	if course_name == "new-course":
		...
		context.course = dontmanage._dict()
...
def set_course_context(context, course_name):
	...
	related_courses = dontmanage.get_all(
	...
	membership = get_membership(course.name, dontmanage.session.user)
...
def get_user_interest(course):
	return dontmanage.db.count(
		"LMS Course Interest", {"course": course, "user": dontmanage.session.user}
	)
