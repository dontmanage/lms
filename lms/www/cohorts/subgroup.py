...
def get_context(context):
	...
	cohort = utils.get_cohort(course, dontmanage.form_dict["cohort"])
	subgroup = utils.get_subgroup(cohort, dontmanage.form_dict["subgroup"])

	if not subgroup:
		context.template = "www/404.html"
		return

	page = dontmanage.form_dict.get("page")
	is_mentor = subgroup.is_mentor(dontmanage.session.user)
	is_admin = (
		cohort.is_admin(dontmanage.session.user) or "System Manager" in dontmanage.get_roles()
	)
	...
	if page not in page_names:
		dontmanage.local.flags.redirect_location = subgroup.get_url() + "/mentors"
