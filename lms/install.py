...
def add_pages_to_nav():
	...
	for page in pages:
		...
		if not dontmanage.db.exists("Top Bar Item", filters):
			dontmanage.get_doc(
				{
...
def delete_lms_roles():
	...
	for role in roles:
		if dontmanage.db.exists("Role", role):
			dontmanage.db.delete("Role", role)
...
def create_course_creator_role():
	if not dontmanage.db.exists("Role", "Course Creator"):
...
def create_moderator_role():
	if not dontmanage.db.exists("Role", "Moderator"):
		role = dontmanage.get_doc(
			{
				"doctype": "Role",
				"role_name": "Moderator",
...
def delete_custom_fields():
	...
	for field in fields:
		dontmanage.db.delete("Custom Field", {"fieldname": field})
