...
def get_profile_url_prefix():
	hooks = dontmanage.get_hooks("profile_url_prefix") or ["/users/"]
	return hooks[-1]
...
class ProfileRedirectPage(BaseRenderer):
	...
	def render(self):
		...
		dontmanage.flags.redirect_location = get_profile_url_prefix() + username
...
class ProfilePage(BaseRenderer):
	...
	def can_render(self):
		...
		routes = [rule["to_route"] for rule in dontmanage.get_hooks("website_route_rules")]
...
def render_portal_page(path, **kwargs):
	dontmanage.form_dict.update(kwargs)
